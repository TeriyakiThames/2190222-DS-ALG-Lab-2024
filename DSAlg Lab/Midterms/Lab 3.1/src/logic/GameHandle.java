...
public class GameHandle {
		...
		while (playerStandUser.getCurrentHp() > 0 && enemyStandUser.getCurrentHp() > 0) {
			...
			switch (command) {
				case 1:
					...
					// int damage = DamageCalculate(Attacker, Defender,StandAttacker,
					// StandDefender);
					System.out.println(Attacker.getName() + " Attack " + Defender.getName());
					// Attack(Defender , damage);
					System.out.println(
							Defender.getName() + " Take " + damage + " damage / " + Defender.getCurrentHp()
									+ " HP left");
					if (Defender.isGuard()) {
	...
	private static int PlayerCommand() {
		...
		System.out.println(
				"3) Use Stand ( " + playerStand.getCurrentChargeTurn() + " / " + playerStand.getMaxChargeTurn() + " )");
		System.out.println("****************************");
