...
public class BST {
	...
	private int height(BSTNode node) {
		...
		// Return the height of the tree from this node
		return 1 + Math.max(leftHeight, rightHeight);
	}

	public void makeBalancedTree() {
		TreeIterator currentItr = new TreeIterator(root);
		if (isBalanced(currentItr.currentNode)) {
			return;
		}
		...
		// Clear previous tree and rebuild it
		this.makeEmpty();
		int listSize = orderList.size();
		this.size = listSize;
		this.root = buildNewTree(orderList, 0, (listSize - 1));

	}

	private void inOrderTraversal(BSTNode node, ArrayList<Integer> orderList) {
		if (node == null) {
			return;
		}

		// Recursively travel through the left and right subtrees
		inOrderTraversal(node.left, orderList);
		orderList.add(node.data);
		inOrderTraversal(node.right, orderList);
	}

	private BSTNode buildNewTree(ArrayList<Integer> orderList, int left, int right) {
		if (left > right) {
			return null;
		}

		// Find middle to create the new root
		int middle = left + (right - left) / 2;
		BSTNode newRoot = new BSTNode(orderList.get(middle));

		// Build the left/right subtree from the left/right half of the list
		newRoot.left = buildNewTree(orderList, left, (middle - 1));
		newRoot.right = buildNewTree(orderList, (middle + 1), right);
		return newRoot;
	}
