public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		// Case 1: Subtree of its root is null
		if (subtree == null || subtree.root == null) {
			return;
		}

		// Case 2: BST root is null
		if (this.root == null) {
			this.root = subtree.root;
			this.size = subtree.size;
		}

		// Case 3: Add subtree to the left of BST
		// findMax and findMin returns type Iterator, so we cast it to TreeIterator
		// so that we can get the currentNode property
		BSTNode subtreeMaxNode = ((TreeIterator) findMax(subtree.root)).currentNode;
		BSTNode currentMinNode = ((TreeIterator) findMin(this.root)).currentNode;

		// If the max value of the subtree is less than the min value of the tree,
		// add the subtree to the tree and adjust the size accordingly
		if (subtreeMaxNode.data < currentMinNode.data) {
			currentMinNode.left = subtree.root;
			subtree.root.parent = currentMinNode;
			this.size += subtree.size;
		}
	}
