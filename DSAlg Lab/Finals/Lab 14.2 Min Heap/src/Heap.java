public class Heap {
	...
	public static boolean isAHeap(Heap h) {
		// Calculate the number of loops we need from h.size
		int maxIndex = (h.size - 1) / 2;
		int[] data = h.mData;
		...
		for (int i = 0; i <= maxIndex; i++) {
			...
			// Check if current node is greater than left child
			if (leftChild < h.size && data[i] > data[leftChild]) {
				return false;
			}

			// Check if current node is greater than right child
			if (rightChild < h.size && data[i] > data[rightChild]) {
				return false;
			}
		}
		return true;
