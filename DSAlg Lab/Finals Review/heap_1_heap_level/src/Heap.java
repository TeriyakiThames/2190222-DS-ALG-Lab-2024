public class Heap {
	...
	public int[] atLevel(int k) {
		int start = (int) Math.pow(2, k) - 1; // 2^k - 1
		int end = (int) Math.pow(2, k + 1) - 2; // 2^(k+1) - 2
		
		// Return if start is out of bounds
		if (start >= size) {
			return new int[0];
		}

		// Adjust the end index to be within bounds
		end = Math.min(end, size - 1);
		
		// Collect the elements within the range
		int[] results = new int[(end - start + 1)];
		for (int i = start; i <= end; i++) {
			results[i - start] = mData[i];
		}
		return results;
	}
